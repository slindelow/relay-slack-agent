...
| 7 — Question classified | ✅ PASS | sofialindelow | 2026-06-24 | Customer message in #test-customer → classified → question created (`/relay pulse TestCo` shows 1 open) → CSM DM alert delivered. Required fixes: worker event-loop (`cc3f63d`), `PYTHONPATH=/app` for `classifier` import, model IDs → Haiku 4.5/Sonnet 4.6 (`d5bf4a6`), assign account owner via `/relay register … @owner`, and enable Slack **Messages Tab** (was disabled → `messages_tab_disabled` blocked DMs). |
| 8 — SLA timer | ⏳ PENDING | — | — | Wait 2min after step 7; `/relay pulse TestCo`; no premature breach alert. |
| 9 — Claim and draft | ✅ PASS | sofialindelow | 2026-06-24 | Draft generated via App Home "Generate draft" → routed through MCP `draft_generation` tool → Sonnet 4.6 → Draft row. Surfaced via new "Drafts Ready for Review" App Home section (`f1152d9`) + "Review draft" button; modal opens after fixing invalid button style (`36bb082`). Note: Claim only sets state (does not auto-generate draft — gap vs spec; draft is a separate action). |
| 10 — Send response | ✅ PASS (UX issue) | sofialindelow | 2026-06-24 | Send posts the response into #test-customer as the RELAY bot and resolves the question. **Known UX issue:** customer-facing message reads `Posted by RELAY on behalf of @<rawUserID> after their approval` ([draft_actions.py:275](../../relay/slack/draft_actions.py)) — shows the raw Slack user ID (no display name / not a real mention) and exposes the internal "after their approval" framing to the customer. Needs a cleaner client-facing format. |
