...
2. Run `/relay add #test-channel TestCo enterprise @owner`
...
| 3 — Channel registered | ✅ PASS | sofialindelow | 2026-06-24 | `/relay add #test-customer TestCo enterprise @owner` → channel registered for account "TestCo". Required two fixes: (1) enable "Escape channels, users, and links" on the `/relay` slash command in Slack app settings; (2) parser fix `1c27eeb` to accept escaped channel mention `<#C123>` without `\|name` suffix. |
...
| 7 — Question classified | ✅ PASS | sofialindelow | 2026-06-24 | Customer message in #test-customer → classified → question created (`/relay pulse TestCo` shows 1 open) → CSM DM alert delivered. Required fixes: worker event-loop (`cc3f63d`), model IDs → Haiku 4.5/Sonnet 4.6 (`d5bf4a6`), assign account owner via `/relay add … @owner`, and enable Slack **Messages Tab** (was disabled → `messages_tab_disabled` blocked DMs). `PYTHONPATH=/app` is now exported by `scripts/entrypoint.sh` (2026-06-25). |
| 8 — SLA timer | 🟢 READY (code complete; awaiting live click-through) | — | 2026-06-30 | `/relay pulse <Account>` now lists each open question with an urgency dot and a **"waiting &lt;age&gt;"** label (`92be8ae`, `format_age` helper). SLA timing verified by construction: a fresh question has `next_alert_at=NULL` → first poll (≤60s) sends the Step 7 alert, then schedules the next alert 240 min out (escalation window), so no premature breach alert can fire in a 2-min window. Live verification recipe in `docs/DEMO_SCRIPT.md` (Step 8 section). |
...
**Resume from:** Open the "RELAY Beta" workspace in Slack → invite RELAY to a test channel → `/relay add #test-channel TestCo enterprise @owner`
